        ...
        for filename in filenames:
            if str(filename).endswith(".txt"):  # and str(filename).startswith(study):
                with open(path + filename, 'r') as test_file:
                    ...
                    #         break
                    #     index += 1
                    # for i in range(index, len(testSequence)):
                    #     if i == len(testSequence) - 1:
                    #         patientEndIndex = len(testSequence)
                    #         break
                    #     if testSequence[i] == "-" or i == len(testSequence) - 1:
                    #         patientEndIndex = index
                    #         break
